...
namespace annotator {
  ...
  void BagPlayer::packetReady(const FFMPEGPacketConstPtr &pkt) {
    rawStream_.write((const char *)(&pkt->data[0]), pkt->data.size());
  }

  BagPlayer::CopyThread::CopyThread(
  ...
  void BagPlayer::startCopyThreads(int w, int h, int size) {
  ...
  void BagPlayer::writeVideoFrame(const cv::Mat &img, const std_msgs::Header &header) {
    if (!encoder_.isInitialized()) {
      encoder_.initialize(img.cols, img.rows,
                          boost::bind(&BagPlayer::packetReady, this, _1));
      if (!encoder_.isInitialized()) {
  ...
  void BagPlayer::syncCallback(const std::vector<ImageConstPtr> &msgs) {
    if (msgs.size() == 0) {
  ...
  void BagPlayer::Session::decodeThread() {
    std::unique_lock<std::mutex> lock(mutex_);
    while (!threadStopSignaled_ && ros::ok()) {
